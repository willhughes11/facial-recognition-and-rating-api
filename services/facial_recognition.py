...
class FacialRecognition():
        ...
        for i in range(len(matches)):
            for x in range(len(matches)):
                if not matches[x]['matched']:
                    result = DeepFace.verify(matches[i]['image'],matches[x]['image'],enforce_detection=False,prog_bar=False)
                    if result['verified']:
                        if matches[i]['matched']:
                            ...
                            if i == 0:
                                matches[x]['matched'] = True
                                matches[x]['matched_photo'] = f'image_{matches[i]["image_id"]}'
                            else:
    ...
    def compare_faces(self,images):
        ...
        z = 0
        for image in images:
            matches.append({'image': image, 'image_id': z, 'matched': False, 'matched_photo': None})
            z += 1
        ...
        filtered_matches = list(filter(lambda d: d['matched_photo'] in [most_common], matches))
    ...
    def analyze_images(self, images):
        analyzed = []
        for image in images:
            obj = DeepFace.analyze(image,actions = ['race'],enforce_detection=False, prog_bar=False)
            analyzed.append(obj)
